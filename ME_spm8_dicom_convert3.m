...
%    fnames{i} = getfilelocation(hdr{i},root_dir,'f',format);
    fnames{i} = root_dir; %% --- Changed by MELLIOTT - name for output file is user-provided %%
...
fnames = cell(length(hdr),1);
if (length(hdr) > 1)   % handles multiple 3D volumes MELLIOTT
    fprintf('Modifying output filenames for %1d volumes...\n',length(hdr));
    [path,root,ext] = fileparts(root_dir);
end
for i=1:length(hdr)
    if (length(hdr) > 1)
        root_dir = sprintf('%s%s%s_%3.3d%s',path,filesep(),root,i,ext);  % MELLIOTT
    end
...
fname = root_dir; %% --- Changed by MELLIOTT - name for output file is user-provided %%
...
%% MElliott - changed to force uint 16-bit
%%dt     = determine_datatype(hdr{1});
...
for i=1:length(hdr),
 ...
 %       disp(['Cant find suitable filename info for "' hdr{i}.Filename '".']);    % MELLIOTT
 ...
 %           disp('Setting InstanceNumber to 1');
        ...
        if ~isfield(hdr{i},'PatientID')   % MELLIOTT - added this whole check!! else no images!!
 %           disp('Setting PatientID to "xxx"');
            hdr{i}.PatientID='xxx';
            images = [images(:)',hdr(i)];
        end;
    else
