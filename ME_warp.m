...
%   $Revision: 5.21.4.4 $  $Date: 2003/08/23 05:54:46 $
...
case 3
    ...
    elseif (isequal(size(varargin{1}), size(varargin{2})) && ...
        ...
        % warp(R,G,B)
	warning(wid,'%s %s','WARP(R,G,B) is an obsolete syntax.',...
...
%%        % warp(z,R,G,B)
%%	warning(wid,'%s %s','WARP(z,R,G,B) is an obsolete syntax.',...
%%                'Use a three dimensional array to represent RGB image.');
...
case 6
    % warp(x,y,z,R,G,B)    OBSOLETE
    warning(wid,'%s %s','WARP(x,y,z,R,G,B) is an obsolete syntax. ',...
            'Use a three dimensional array to represent RGB image.');    
